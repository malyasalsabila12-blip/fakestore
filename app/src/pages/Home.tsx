      ...
      <div className="mx-auto max-w-7xl p-4 md:p-8">
        ...
        <div className="relative w-full max-w-xs group">
          <button 
            ...
            className={`absolute -left-8 top-2 transition-colors ${isListening ? 'text-red-600 animate-pulse' : 'text-zinc-400 hover:text-black'}`}
            title="Voice search"
          >
            <span className="material-icons text-lg">{isListening ? 'mic' : 'mic_none'}</span>
          </button>
          <input
            ref={inputRef}
            type="text"
            ...
            className={`w-full border-b border-black py-2 text-sm outline-none bg-transparent pl-2`}
            data-test="search-input"
          />
          {searchQuery ? (
            <button 
              onClick={() => setSearchQuery('')}
              className="absolute right-0 top-2 text-zinc-400 hover:text-black transition-colors"
            >
