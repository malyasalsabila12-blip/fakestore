...
async function generateAuthReport(testResults) {
    const doc = new PDFDocument();
    const reportPath = path.join(__dirname, "reports", "auth-flow-report.pdf");
    ...
    doc.fontSize(25).text("Authentication Flow Test Report", { align: "center" });
    doc.moveDown();
    ...
    if (testResults.length === 0) {
        doc.fontSize(12).text("No test results found for Authentication Flow.", { indent: 20 });
    }
...
function findAuthSpecs(suites, results = []) {
    suites.forEach(suite => {
        if (suite.title === "Authentication Flow") {
            // Found the right suite, collect all specs inside it
            collectSpecs(suite, results);
        } else if (suite.suites) {
            findAuthSpecs(suite.suites, results);
        }
    });
    return results;
}
...
const playwrightJsonPath = path.join(__dirname, "..", "app", "playwright-report", "results_utf8.json");
if (fs.existsSync(playwrightJsonPath)) {
    ...
    try {
        ...
        const authResults = findAuthSpecs(data.suites);
